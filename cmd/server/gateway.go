...
import (
	...
	_ "github.com/sirjager/go_emails/docs/statik"
	"github.com/sirjager/go_emails/pkg/server"

	rpc "github.com/sirjager/rpcs/emails/go"
)

func RunGatewayServer(srvr *server.RPCEmailsServer, logger zerolog.Logger, config cfg.Config, errs chan error) {
	...
	err := rpc.RegisterEmailsHandlerServer(ctx, grpcMux, srvr)
	if err != nil {
		errs <- err
